...
class fenReliefsDisks():
    def __init__(self, fen_param, frame_param):
        ...
        getter_rel_disk = m_access_to_settings.accessToSettings('reliefs.ini')
        self.rel_widg = getter_rel_disk.get('disks', 'widg')
        self.rel_partition = getter_rel_disk.get('disks', 'partition')
        self.rel_barre = getter_rel_disk.get('disks', 'barre')
        self.rel_write = getter_rel_disk.get('disks', 'write')
        self.rel_read = getter_rel_disk.get('disks', 'read')
        self.liste_pour_ini = ['widg', 'partition', 'barre', 'write', 'read']
        self.creer_params_disks()

    def creer_params_disks(self):
        liste_reliefs.afficherListeReliefs(self.frame_param, self.liste_reliefs)
        list_labels = []
        text = ['Relief du widget: ', 'Relief des partitions: ', 'Relief des barres: ', 'Relief zone "write": ',
                'Relief zone "read"']
        reliefs = [self.rel_widg, self.rel_partition, self.rel_barre, self.rel_write, self.rel_read]
    ...
    def appliquer(self):
        ...
        for var in self.textvar:
            ...
            if var == 'unchange':
                ...
                getter_set.set('disks', self.liste_pour_ini[i], var)
